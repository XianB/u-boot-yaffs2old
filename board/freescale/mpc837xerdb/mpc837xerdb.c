...
{
#if defined(CONFIG_FSL_SERDES) || defined(CONFIG_MMC)
...
#ifdef CONFIG_MMC
	immr->sysconf.sicrl &= ~SICRL_USB_B_MASK;
	immr->sysconf.sicrl |= SICRL_USB_B_SD;

	immr->sysconf.sicrh &= ~SICRH_SPI_MASK;
	immr->sysconf.sicrh |= SICRH_SPI_SD;
#endif
